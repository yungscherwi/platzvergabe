function printpage() {
 ...
 //Öffnen von zwei neuen Tabs nach klicken von "Abschicken"
  ...
  function showReihen(int){
	...
	//wenn gescrollt wird, führe myFunction aus
    window.onscroll = function() {myFunction()};

    var navbar = document.getElementById("navigation-container");
    var sticky = navbar.offsetTop; //navbar oben festsetzen

    function myFunction() {
        if (window.pageYOffset >= sticky) { //wenn Navbar durch scrollen verschwindet
        navbar.classList.add("sticky") //durch sticky navbar weiterhin oben anzeigen
        }
        else {
        navbar.classList.remove("sticky"); //sonst remove sticky
        }
    }
